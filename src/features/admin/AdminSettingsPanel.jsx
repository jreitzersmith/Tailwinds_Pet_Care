...
// Lets the admin edit each service's base price and control whether it bills
// per-pet (services.price_per_pet).
function PricingEditor() {
  const [services,    setServices]    = useState([]);
  const [loading,     setLoading]     = useState(true);
  const [error,       setError]       = useState(null);
  const [savingId,    setSavingId]    = useState(null);
  const [priceDrafts, setPriceDrafts] = useState({}); // { [id]: string being edited }
  const [priceErrors, setPriceErrors] = useState({}); // { [id]: error text }
  ...
  // Current text shown in the base-price input: the in-progress draft if the
  // admin is editing it, otherwise the saved value (blank means "Quote").
  function priceDraftValue(svc) {
    if (priceDrafts[svc.id] !== undefined) return priceDrafts[svc.id];
    return svc.base_price == null ? '' : String(svc.base_price);
  }

  function handlePriceChange(svc, value) {
    setPriceDrafts(prev => ({ ...prev, [svc.id]: value }));
    if (priceErrors[svc.id]) setPriceErrors(prev => ({ ...prev, [svc.id]: null }));
  }

  async function commitBasePrice(svc) {
    if (priceDrafts[svc.id] === undefined) return; // nothing changed
    const raw = priceDrafts[svc.id].trim();

    let nextPrice;
    if (raw === '') {
      nextPrice = null; // Blank = quote-based, no fixed price
    } else {
      const parsed = Number(raw);
      if (Number.isNaN(parsed) || parsed < 0) {
        setPriceErrors(prev => ({ ...prev, [svc.id]: 'Enter a valid non-negative price.' }));
        return;
      }
      nextPrice = Math.round(parsed * 100) / 100;
    }

    const clearDraft = () => setPriceDrafts(prev => {
      const next = { ...prev };
      delete next[svc.id];
      return next;
    });

    if (nextPrice === svc.base_price) { clearDraft(); return; }

    const prevPrice = svc.base_price;
    setSavingId(svc.id);
    setServices(prev => prev.map(s => s.id === svc.id ? { ...s, base_price: nextPrice } : s));
    const { error: err } = await supabase
      .from('services')
      .update({ base_price: nextPrice })
      .eq('id', svc.id);
    setSavingId(null);

    if (err) {
      setServices(prev => prev.map(s => s.id === svc.id ? { ...s, base_price: prevPrice } : s));
      setPriceErrors(prev => ({ ...prev, [svc.id]: `Save failed: ${err.message}` }));
    } else {
      clearDraft();
    }
  }
    ...
    <div style={pe.wrap}>
      <div style={pe.intro}>
        ...
        <p style={pe.introDesc}>
          Edit a service&apos;s <strong>Base Price</strong> and press Enter or click away to
          save — leave it blank for quote-based services. Toggle <strong>Bill Per Pet</strong> to
          charge the base price for every pet on a visit; when off, the service is charged
          once per visit regardless of pet count.
        </p>
      </div>
              ...
              {list.map(svc => (
                <tr key={svc.id}>
                  ...
                  <td style={{ ...pe.td, textAlign: 'right' }}>
                    <div style={pe.priceCell}>
                      <span style={pe.priceDollar}>$</span>
                      <input
                        type='number'
                        step='0.01'
                        min='0'
                        style={pe.priceInput}
                        value={priceDraftValue(svc)}
                        placeholder='Quote'
                        disabled={savingId === svc.id}
                        onChange={e => handlePriceChange(svc, e.target.value)}
                        onBlur={() => commitBasePrice(svc)}
                        onKeyDown={e => { if (e.key === 'Enter') e.target.blur(); }}
                      />
                      {savingId === svc.id && <span style={pe.savingNote}>saving…</span>}
                    </div>
                    {priceErrors[svc.id] && (
                      <p style={pe.priceError}>{priceErrors[svc.id]}</p>
                    )}
                  </td>
...
const pe = {
  ...
  priceCell:  { display: 'inline-flex', alignItems: 'center', justifyContent: 'flex-end', gap: '0.4rem' },
  priceDollar:{ color: '#999' },
  priceInput: {
    width: '80px', textAlign: 'right', padding: '0.3rem 0.4rem', borderRadius: '5px',
    border: `1px solid #d5e2ee`, fontFamily: FONTS.body, fontSize: '0.875rem',
    color: COLORS.black, outline: 'none', boxSizing: 'border-box',
  },
  priceError: { fontFamily: FONTS.body, fontSize: '0.72rem', color: COLORS.red, margin: '0.2rem 0 0', textAlign: 'right' },
